...
class DatabasesControllerTest < ActionController::TestCase
  ...
  test "should create database" do
    assert_difference('Database.count') do
      post :create, database: { name: @database.name, port: @database.port, db_type: @database.db_type, version: @database.version }
    end
  ...
  test "should update database" do
    put :update, id: @database, database: { name: @database.name, port: @database.port, db_type: @database.db_type, version: @database.version }
    assert_redirected_to database_path(assigns(:database))
  end
